...
| `rules/` | 静态规则（CI 生成）；外部规则由容器启动时 wget 下载 |
...
| 变量 | 默认 | 说明 |
|------|------|------|
| TZ | Asia/Shanghai | 时区，crond 04:30 重启 |
...
- **规则更新**：容器内 crond 每日 04:30 `kill 1`，依赖 Docker `restart` 或 RouterOS scheduler 04:35 重启，再次拉规则。
- **AI 同步**：每 2 分钟 SSH 将 `ai-list.txt` 解析出的 IP 写入 RouterOS `ai-sgp`（comment=mosdns-ai）。
- **改配置/规则**：改 `updater/*` 或 `sites.yaml`/`config.base.yaml` 后 push，CI 生成 `configs/`、`rules/`、`scripts/ai-sgp-sync.rsc`。
...
- **generate.yml**：上述源文件变更时生成 configs、rules、ai-sgp-sync.rsc 并提交。
